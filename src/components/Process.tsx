...
export const Process: React.FC = () => {
  ...
  return (
    <section id="process" className="py-20 relative bg-[#F1EEFF]/30 dark:bg-[#09090F]/60 border-y border-[#E5E1F5] dark:border-[#28243A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:grid grid-cols-7 gap-2 mb-12 relative">
          <div className="absolute top-1/2 left-4 right-4 h-0.5 bg-[#E5E1F5] dark:bg-[#28243A] -translate-y-1/2 z-0" />
          {steps.map((step) => {
              ...
              >
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold font-mono transition-all ${
                    isActive
                      ? 'bg-[#7C3AED] text-white shadow-lg shadow-[#7C3AED]/30 scale-110 ring-4 ring-[#F1EEFF] dark:ring-[#171525]'
                      : 'bg-white dark:bg-[#11111A] text-[#11111A] dark:text-[#A9A7B8] border border-[#E5E1F5] dark:border-[#28243A] group-hover:border-[#7C3AED]'
                  }`}
                >
                  0{step.num}
                </div>
                <span
                  className={`mt-3 text-xs font-bold text-center max-w-[100px] line-clamp-2 transition-colors ${
                    isActive
                      ? 'text-[#7C3AED] dark:text-[#A855F7] font-black'
                      : 'text-[#11111A] dark:text-[#A9A7B8] group-hover:text-[#7C3AED] dark:group-hover:text-[#A855F7]'
                  }`}
                >
                  {step.title}
                </span>
        ...
        {steps.map((step) => {
          ...
          return (
            <div
              key={step.num}
              className="glass-panel p-8 rounded-2xl border border-[#E5E1F5] dark:border-[#28243A] shadow-xl max-w-4xl mx-auto space-y-6 animate-fade-in"
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-[#E5E1F5] dark:border-[#28243A]">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#7C3AED] to-[#A855F7] text-white flex items-center justify-center shadow-lg shadow-[#7C3AED]/25">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#5F6070] dark:text-[#A9A7B8] font-medium">{step.subtitle}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg bg-[#F1EEFF] dark:bg-[#171525] hover:bg-[#E5E1F5] dark:hover:bg-[#28243A] text-xs font-bold text-[#11111A] dark:text-[#F8F7FF] border border-[#E5E1F5] dark:border-[#28243A] disabled:opacity-40 cursor-pointer"
                  >
                    Previous
                  </button>
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-xs font-bold disabled:opacity-40 shadow-sm cursor-pointer shadow-[#7C3AED]/20"
                  >
                    Next Step
                  </button>
                </div>
              ...
              <p className="text-sm sm:text-base text-[#5F6070] dark:text-[#A9A7B8] leading-relaxed font-medium">
                {step.desc}
              </p>

              <div>
                <h4 className="text-xs font-bold font-mono text-[#11111A] dark:text-[#F8F7FF] uppercase tracking-wider mb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {step.deliverables.map((del, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-xl bg-white dark:bg-[#171525] border border-[#E5E1F5] dark:border-[#28243A] text-xs font-bold text-[#11111A] dark:text-[#F8F7FF] flex items-center gap-2"
                    >
        ...
        <div className="lg:hidden space-y-4 mt-8">
          {steps.map((step) => {
            ...
            return (
              <div
                key={step.num}
                className="glass-panel rounded-2xl border border-[#E5E1F5] dark:border-[#28243A] overflow-hidden"
              >
                <button
                  onClick={() => setActiveStep(step.num)}
                  className="w-full p-4 flex items-center justify-between text-left font-bold text-[#11111A] dark:text-[#F8F7FF] cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-xl bg-[#F1EEFF] dark:bg-[#171525] text-[#7C3AED] dark:text-[#A855F7] flex items-center justify-center text-xs font-mono font-bold">
                      0{step.num}
                    </span>
                    <span className="text-sm">{step.title}</span>
                  </div>
                  <ArrowRight
                    className={`w-4 h-4 text-[#7C3AED] dark:text-[#A855F7] transition-transform ${isOpen ? 'rotate-90' : ''}`}
                  />
                </button>
                {isOpen && (
                  <div className="p-4 pt-0 text-xs text-[#5F6070] dark:text-[#A9A7B8] space-y-3 border-t border-[#E5E1F5] dark:border-[#28243A] font-medium">
                    <p className="pt-2">{step.desc}</p>
                    <div className="space-y-1.5">
                      {step.deliverables.map((d, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <CheckCircle2 className="w-3.5 h-3.5 text-[#7C3AED] dark:text-[#A855F7]" />
                          <span className="font-bold text-[#11111A] dark:text-[#F8F7FF]">{d}</span>
                        </div>
                      ))}
                    </div>
                  </div>
